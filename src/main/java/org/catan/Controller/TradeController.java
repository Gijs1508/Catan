...
public class TradeController implements Observable {
    ...
    @FXML public void buyDevelopmentCard() {
        ...
        if(getInventoryCards()[2] >= 1 && getInventoryCards()[3] >= 1 && getInventoryCards()[4] >= 1 && App.getClientPlayer().isTurn()){
            // If player has enough resources
        ...
        else if (App.getClientPlayer().isTurn()){
            ScreenController.getInstance().showAlertPopup();
    ...
    public void sendTrade() throws IOException {
        if(tradeType == "bank" && App.getClientPlayer().isTurn()){
            ...
            resetTrade();
        } else if(tradeType == "player" && App.getClientPlayer().isTurn()){
...
//            App.tradePopUp();
    ...
    private void giveResource(Label resource, int inventoryIndex){
        ...
        if(tradeType == "player"){
            ...
            int cost = App.getCurrentGame().turnPlayerGetter().getCostOf(resourceType); // Gets the player's cost for the resource type
